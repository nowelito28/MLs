...
def createLegends():
    ...
    legend_algebraic[17] = (
        "i_K in component time_dependent_potassium_current (microA_per_cm2)"
    )
    legend_algebraic[21] = (
        "i_K1 in component time_independent_potassium_current (microA_per_cm2)"
    )
    legend_algebraic[23] = (
        "i_Kp in component plateau_potassium_current (microA_per_cm2)"
    )
    ...
    legend_states[4] = (
        "Cai in component intracellular_calcium_concentration (millimolar)"
    )
    ...
    legend_algebraic[4] = (
        "alpha_d in component slow_inward_current_d_gate (per_millisecond)"
    )
    legend_algebraic[11] = (
        "beta_d in component slow_inward_current_d_gate (per_millisecond)"
    )
    legend_algebraic[5] = (
        "alpha_f in component slow_inward_current_f_gate (per_millisecond)"
    )
    legend_algebraic[12] = (
        "beta_f in component slow_inward_current_f_gate (per_millisecond)"
    )
    legend_constants[19] = (
        "g_K in component time_dependent_potassium_current (milliS_per_cm2)"
    )
    legend_constants[20] = (
        "E_K in component time_dependent_potassium_current (millivolt)"
    )
    legend_constants[12] = (
        "PR_NaK in component time_dependent_potassium_current (dimensionless)"
    )
    ...
    legend_states[7] = (
        "X in component time_dependent_potassium_current_X_gate (dimensionless)"
    )
    legend_algebraic[16] = (
        "Xi in component time_dependent_potassium_current_Xi_gate (dimensionless)"
    )
    legend_algebraic[6] = (
        "alpha_X in component time_dependent_potassium_current_X_gate (per_millisecond)"
    )
    legend_algebraic[13] = (
        "beta_X in component time_dependent_potassium_current_X_gate (per_millisecond)"
    )
    legend_constants[21] = (
        "E_K1 in component time_independent_potassium_current (millivolt)"
    )
    legend_constants[22] = (
        "g_K1 in component time_independent_potassium_current (milliS_per_cm2)"
    )
    legend_algebraic[20] = (
        "K1_infinity in component time_independent_potassium_current_K1_gate (dimensionless)"
    )
    legend_algebraic[18] = (
        "alpha_K1 in component time_independent_potassium_current_K1_gate (per_millisecond)"
    )
    legend_algebraic[19] = (
        "beta_K1 in component time_independent_potassium_current_K1_gate (per_millisecond)"
    )
    legend_constants[23] = "E_Kp in component plateau_potassium_current (millivolt)"
    legend_constants[15] = (
        "g_Kp in component plateau_potassium_current (milliS_per_cm2)"
    )
    ...
    legend_rates[7] = (
        "d/dt X in component time_dependent_potassium_current_X_gate (dimensionless)"
    )
    legend_rates[4] = (
        "d/dt Cai in component intracellular_calcium_concentration (millimolar)"
    )
...
def initConsts():
    ...
    constants[18] = ((constants[0] * constants[1]) / constants[2]) * log(
    ...
    constants[20] = ((constants[0] * constants[1]) / constants[2]) * log(
        (constants[13] + constants[12] * constants[10])
        / (constants[14] + constants[12] * constants[11])
    )
    constants[21] = ((constants[0] * constants[1]) / constants[2]) * log(
...
def computeRates(voi, states, constants):
    ...
    algebraic[1] = (0.320000 * (states[0] + 47.1300)) / (
    ...
    algebraic[2] = custom_piecewise(
        [
            ...
            0.135000 * exp((80.0000 + states[0]) / -6.80000),
            True,
            0.00000,
        ]
    )
    ...
    rates[4] = (-0.000100000 / 1.00000) * algebraic[15] + 0.0700000 * (
        0.000100000 - states[4]
    )
    ...
    algebraic[16] = custom_piecewise(
        [
            greater(states[0], -100.000),
    ...
    algebraic[17] = (
        constants[19] * states[7] * algebraic[16] * (states[0] - constants[20])
    )
    algebraic[18] = 1.02000 / (
        1.00000 + exp(0.238500 * ((states[0] - constants[21]) - 59.2150))
    )
    algebraic[19] = (
        0.491240 * exp(0.0803200 * ((states[0] + 5.47600) - constants[21]))
        + 1.00000 * exp(0.0617500 * (states[0] - (constants[21] + 594.310)))
